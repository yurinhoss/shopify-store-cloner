import { COUNTRY_CURRENCIES } from './currencies.js';
...
export const COUNTRIES = Object.entries(regions).flatMap(([region,codes]) => codes.split(' ').map(code => {
  ...
  return {code, name:names.of(code), region, language, currency:COUNTRY_CURRENCIES[code]?.[0] || null, excluded:EXCLUDED.has(code)};
...
export function resolveLocale(country, locales, override) {
  ...
  if (!english) return {error:'Inglês não está publicado nesta loja. Publique o inglês no admin antes de configurar os mercados.'};
  if (override && !published.includes(override)) return {error:'O idioma escolhido não está publicado nesta loja.'};
  // Nunca substitui português de Portugal por português do Brasil (nem o contrário).
  ...
  // Escolha AUTOMÁTICA: idioma local publicado; se não houver, inglês (que já foi validado acima).
  // Nada bloqueia a aplicação — o override manual continua disponível, mas é opcional.
  const primary=override || local || english;
  const source=override?'manual':local?'local':'english';
  return {
    primary,
    english,
    alternate: primary===english?[]:[english],
    source,
    warning: source==='english' ? `Idioma local (${country.language}) não publicado; usando inglês automaticamente.` : null,
  };
}
...
export function resolveCurrency(country, supported, shopCurrency) {
  const candidates=COUNTRY_CURRENCIES[country.code] || [];
  const currency=candidates.find(code=>supported.includes(code)) || shopCurrency;
  const warning=candidates[0]===currency ? null : candidates.length
    ? `Moeda ${candidates[0]} indisponível na API Shopify; ${currency} definido automaticamente${currency===shopCurrency?' (moeda da loja)':''}.`
    : `Território sem moeda própria cadastrada; usando ${currency}, moeda da loja.`;
