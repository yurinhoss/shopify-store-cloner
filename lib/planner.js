...
export async function ensureMarket(call,row) {
  ...
  return `${row.currency} · ${row.locale.primary}${row.locale.alternate.length?' + '+row.locale.alternate.join(', '):''} · ${urls.join(' | ')||'/'+row.locale.primary.toLowerCase()+'-'+suffix}`;
}
