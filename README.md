...
2. Generate a preview. Markets reads **that destination's published locales**, every time. Portuguese from Portugal is never silently replaced with Brazilian Portuguese. If the suggested local language is absent, choose a published language, or explicitly choose the button to use English for missing languages. English must itself be published. The tool does not create translations or publish languages.
